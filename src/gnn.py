...
class GNN(pl.LightningModule):
        ...
        if mode != 'eval':
            ...
            if self.state_variables is not None:
                for i, variable in enumerate(self.state_variables):
                    loss_variable = self.criterion(dzdt_net.reshape(z1_norm.shape)[n == 1, i], z1_norm[n == 1, i])
    ...
    def extrac_pass(self, batch, mode):
        ...
        if self.project_name == 'Beam_3D':
            ...
            z_t0, z_t1, edge_index, n, f = batch.x, batch.y, batch.edge_index, batch.n, batch.vel
